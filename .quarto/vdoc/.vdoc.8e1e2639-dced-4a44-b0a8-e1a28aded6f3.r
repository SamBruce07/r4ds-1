...
music |>
  mutate(
    ...
    artist.longitude = replace_na(artist.longitude, 0)
  ) |>
...
artist_locations <- music |>
  ...
  mutate(
    genre = case_when(
      str_detect(str_to_lower(artist.terms), "rock") ~ "rock",
      str_detect(str_to_lower(artist.terms), "pop") ~ "pop",
      str_detect(str_to_lower(artist.terms), "blues") ~ "blues",
      str_detect(str_to_lower(artist.terms), "jazz") ~ "jazz",
      str_detect(str_to_lower(artist.terms), "metal") ~ "metal",
      TRUE ~ NA_character_
    ),
    genre_priority = match(genre, c("rock", "pop", "blues", "jazz", "metal"))
  ) |>
  filter(!is.na(genre)) |>
  group_by(artist.id, artist.latitude, artist.longitude) |>
  slice_min(genre_priority, n = 1, with_ties = FALSE) |>
  ungroup()
...
ggplot() +
  ...
  coord_fixed(1.3) +
  labs(
    title = "Geographic distribution of artists by genre",
    subtitle = "Rock and pop cluster in North America and Europe; other genres are more dispersed.",
    x = "Longitude",
    y = "Latitude"
  )
#
